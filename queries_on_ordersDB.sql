...
count(product_id) as count_product_types,
sum(p.product_quantity_avail*p.product_price) AS inventory_value
from product_class pc
inner join product p on pc.product_class_code
...
## Answer 7.
SELECT ORDER_ID ,SUM(P.LEN*P.WIDTH*P.HEIGHT) AS volume FROM ORDER_ITEMS O
...
HAVING SUM(OI.PRODUCT_QUANTITY) > 0
